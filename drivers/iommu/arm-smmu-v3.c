...
struct arm_smmu_master {
	...
	bool				ats_enabled		:1;
	bool				can_fault		:1;
	bool				stall_enabled		:1;
	bool				prg_resp_needs_ssid	:1;
};
				      ...
				      __le64 *dst)
{
	...
	if (s1_cfg) {
		struct iommu_pasid_table_cfg *cfg = &s1_cfg->tables;
...
static int arm_smmu_handle_evt(struct arm_smmu_device *smmu, u64 *evt)
{
	...
	if (nested) {
		if (arm_smmu_transcode_fault(evt, type, &fault_evt.fault.event)) {
			/*
...
static void arm_smmu_tlb_inv_context(void *cookie)
{
	...
	if (smmu_domain->stage == ARM_SMMU_DOMAIN_S1) {
		__arm_smmu_tlb_inv_asid(smmu_domain, 0,
					smmu_domain->s1_cfg->cd0->arch_id);
	} else {
					...
					enum iommu_dev_features feat)
{
	...
	arm_smmu_disable_pri(dev_iommu_fwspec_get(dev)->iommu_priv);
	iopf_queue_remove_device(master->smmu->evtq.iopf, dev);
	iopf_queue_remove_device(master->smmu->priq.iopf, dev);
				       ...
				       struct iommu_pasid_table_config *cfg)
{
	...
	case IOMMU_PASID_CONFIG_TRANSLATE:
		...
		smmu_domain->s1_cfg->tables.base = cfg->base_ptr;
		smmu_domain->abort = false;
		break;
	default:
		goto out;
